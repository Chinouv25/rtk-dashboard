...
def guardar_perfil_sheet(perfil):
    ...
    try:
        ...
        fila = {
            ...
            "peso":      perfil["peso"],
            "sesiones":  json.dumps(perfil["sesiones"])
        }
        if perfil["nombre"] in nombres:
            idx = nombres.index(perfil["nombre"]) + 2  # +2 por header
            ws.update(f"A{idx}:E{idx}", [[
                fila["nombre"], fila["edad"],
                fila["categoria"], fila["peso"],
                fila["sesiones"]
            ]])
        else:
            # Si hoja vacía agregar header
            if not datos:
...
def val_to_color_custom(v, vmin, vmax, stops_rgb, stops_pos):
    ...
    for i in range(len(stops_pos)-1):
        ...
        if t <= t1:
            f  = (t - t0) / (t1 - t0 + 1e-9)
            c0 = stops_rgb[i]; c1 = stops_rgb[i+1]
...
def metricas_sesion(df, geocercas):
    """Extrae métricas globales y por zona de un dataframe."""
    met = {
...
with tab1:
    with st.sidebar:
        ...
        stop_colors, stop_vals = [], []
        for k in range(5):
            cc, cv = st.columns([2,3])
            with cc:
                sc = st.color_picker(f"C{k+1}", default_colors[k], key=f"sc_{k}")
            with cv:
                pct = st.number_input(
                    f"% pos {k+1}", 0, 100,
...
with tab2:
    ...
    if not perfiles:
        st.info("No hay perfiles creados. Crea el primero arriba.")
    else:
        ...
        with st.expander("📂 Cargar nueva sesión semanal"):
            with st.form("form_sesion", clear_on_submit=True):
                ...
                with c1:
                    ...
                    archivo = st.file_uploader("CSV del recorrido", type=["csv"])
                with c2:
                    nota = st.text_area("Notas de la sesión",
                                        placeholder="Ej: Pista mojada, viento fuerte...",
                                        height=100)

                if st.form_submit_button("💾 Guardar sesión") and semana and archivo:
                    df_ses = load_csv(archivo.read(), archivo.name)
                    met    = metricas_sesion(df_ses,
                    ...
                    st.success(f"Sesión '{semana}' guardada para {deportista_sel}.")
                    st.rerun()
        ...
        if p["sesiones"]:
            ...
            def color_evol(col):
                if col.name in ["Vel máx","Vel prom","Salto máx"]:
                    ...
                    return [
                        f"background-color:{'#1a4a1a' if v==mx else '#4a1a1a' if v==mn else ''}"
                        for v in col
                    ]
                return [""] * len(col)
            ...
            with col_g1:
                fig_vmax = px.line(df_evol, x="Semana", y="Vel máx",
                                   title="Velocidad máxima por semana",
                                   markers=True,
                                   color_discrete_sequence=["#378ADD"])
                fig_vmax.update_layout(height=250, margin=dict(t=40,b=20),
                                       plot_bgcolor="rgba(0,0,0,0)",
                                       paper_bgcolor="rgba(0,0,0,0)")
                st.plotly_chart(fig_vmax, use_container_width=True)

                fig_jump = px.line(df_evol, x="Semana", y="Salto máx",
                                   title="Salto máximo por semana",
                                   markers=True,
                                   color_discrete_sequence=["#1D9E75"])
                fig_jump.update_layout(height=250, margin=dict(t=40,b=20),
                                       plot_bgcolor="rgba(0,0,0,0)",
                                       paper_bgcolor="rgba(0,0,0,0)")
                st.plotly_chart(fig_jump, use_container_width=True)

            with col_g2:
                fig_vprom = px.line(df_evol, x="Semana", y="Vel prom",
                                    title="Velocidad promedio por semana",
                                    markers=True,
                                    color_discrete_sequence=["#EF9F27"])
                fig_vprom.update_layout(height=250, margin=dict(t=40,b=20),
                                        plot_bgcolor="rgba(0,0,0,0)",
                                        paper_bgcolor="rgba(0,0,0,0)")
                st.plotly_chart(fig_vprom, use_container_width=True)

                fig_ttime = px.line(df_evol, x="Semana", y="T total",
                                    title="Tiempo total por semana (s)",
                                    markers=True,
                                    color_discrete_sequence=["#D4537E"])
                fig_ttime.update_layout(height=250, margin=dict(t=40,b=20),
                                        plot_bgcolor="rgba(0,0,0,0)",
                                        paper_bgcolor="rgba(0,0,0,0)")
                st.plotly_chart(fig_ttime, use_container_width=True)

            # Evolución por zona
            zonas_disponibles = set()
            for ses in p["sesiones"].values():
                zonas_disponibles.update(ses.get("zonas",{}).keys())

            if zonas_disponibles:
                st.markdown("#### Evolución por zona")
                zona_sel = st.selectbox("Seleccionar zona:",
                                        sorted(zonas_disponibles),
                                        key="zona_evol")
                df_zona_evol = pd.DataFrame([
                    {
                        "Semana":    s,
                        "Vel máx":   p["sesiones"][s]["zonas"].get(zona_sel,{}).get("vel_max",None),
                        "Vel prom":  p["sesiones"][s]["zonas"].get(zona_sel,{}).get("vel_prom",None),
                        "Salto máx": p["sesiones"][s]["zonas"].get(zona_sel,{}).get("jump_max",None),
                        "T zona":    p["sesiones"][s]["zonas"].get(zona_sel,{}).get("t_zona",None),
                    }
                    for s in semanas
                    if zona_sel in p["sesiones"][s].get("zonas",{})
                ])

                if not df_zona_evol.empty:
                    col_z1, col_z2 = st.columns(2)
                    with col_z1:
                        fig_zv = px.line(df_zona_evol, x="Semana",
                                         y="Vel máx", markers=True,
                                         title=f"Vel máx — {zona_sel}",
                                         color_discrete_sequence=["#378ADD"])
                        fig_zv.update_layout(height=230, margin=dict(t=40,b=20),
                                             plot_bgcolor="rgba(0,0,0,0)",
                                             paper_bgcolor="rgba(0,0,0,0)")
                        st.plotly_chart(fig_zv, use_container_width=True)

                        fig_zt = px.line(df_zona_evol, x="Semana",
                                         y="T zona", markers=True,
                                         title=f"Tiempo zona — {zona_sel}",
                                         color_discrete_sequence=["#D4537E"])
                        fig_zt.update_layout(height=230, margin=dict(t=40,b=20),
                                             plot_bgcolor="rgba(0,0,0,0)",
                                             paper_bgcolor="rgba(0,0,0,0)")
                        st.plotly_chart(fig_zt, use_container_width=True)

                    with col_z2:
                        fig_zvp = px.line(df_zona_evol, x="Semana",
                                          y="Vel prom", markers=True,
                                          title=f"Vel prom — {zona_sel}",
                                          color_discrete_sequence=["#EF9F27"])
                        fig_zvp.update_layout(height=230, margin=dict(t=40,b=20),
                                              plot_bgcolor="rgba(0,0,0,0)",
                                              paper_bgcolor="rgba(0,0,0,0)")
                        st.plotly_chart(fig_zvp, use_container_width=True)

                        fig_zj = px.line(df_zona_evol, x="Semana",
                                         y="Salto máx", markers=True,
                                         title=f"Salto máx — {zona_sel}",
                                         color_discrete_sequence=["#1D9E75"])
                        fig_zj.update_layout(height=230, margin=dict(t=40,b=20),
                                             plot_bgcolor="rgba(0,0,0,0)",
                                             paper_bgcolor="rgba(0,0,0,0)")
            ...
            with st.expander("🗑️ Gestionar sesiones"):
                for semana in list(p["sesiones"].keys()):
                    ...
                    col1.write(
                        f"**{semana}** — Vel máx: {ses['vel_max']:.2f} km/h | "
            ...
            st.info("No hay sesiones registradas. Carga la primera sesión arriba.")
